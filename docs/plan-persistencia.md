...
Encaja con lo que ya existe en el repositorio: la app expone endpoints `async` y
la suite corre en `asyncio_mode = "auto"`, así que un engine y sesiones async
evitan mezclar estilos sync y async, y `asyncpg` es el driver async de
PostgreSQL para SQLAlchemy 2.x.
